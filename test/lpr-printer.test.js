...
test('envía datos RAW y archivo de control mediante LPR', async (t) => {
  ...
  const server = net.createServer((socket) => {
    ...
    function processBuffer() {
      ...
      if (phase === 'control-header') {
        ...
        assert.ok(match);
        assert.match(match[2], /^cfA001TEST-PC$/);
      ...
      if (phase === 'control' && buffer.length >= expectedFileBytes + 1) {
        ...
        assert.match(control, /ldfA001TEST-PC/);
        assert.match(control, /UdfA001TEST-PC/);
      ...
      if (phase === 'data-header') {
        ...
        if (end < 0) return;
        const header = buffer.subarray(0, end).toString('ascii');
        const match = /^\x03(\d+) (\S+)$/.exec(header);
        assert.ok(match);
        assert.match(match[2], /^dfA001TEST-PC$/);
        expectedFileBytes = Number(match[1]);
        buffer = buffer.subarray(end + 1);
        socket.write(Buffer.from([0]));
        phase = 'data';
  ...
  assert.equal(controlReceived, true);
});
